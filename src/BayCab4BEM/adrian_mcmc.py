...
with pm.Model() as model:
	...
	tf = pm.Uniform('tf', lower=0, upper=1, shape=q)
	...
	#X = tt.tile(tf,(n,1))
	t = tt.concatenate([tt.tile(tf,(n,1)),tc], axis=0)
	X = tt.concatenate([x,t], axis=1)
	...
	cov = sigma_eta(X)
	new_cov = tt.set_subtensor(cov[0:n,0:n],cov[0:n,0:n]+sigma_delta(xf)+sigma_e)
