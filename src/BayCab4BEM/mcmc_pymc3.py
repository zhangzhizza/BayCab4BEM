...
class MCMC4Posterior_pymc3(object):
	...
	def build(self, covFuncName = 'covFuncPymcNat'):

		with Model() as mvNormalLikelihood:
			# Define priors
			# For theta
			theta_dist = thetaPriorInfo[0];
			theta = theta_dist('theta', *thetaPriorInfo[1:], shape=self._q);
			# For rho_eta
			rho_eta_dist = rho_etaPriorInfo[0];
			rho_eta = rho_eta_dist('rho_eta', *rho_etaPriorInfo[1:], shape = self._p + self._q);
			beta_eta = -4.0 * tt.log(rho_eta);
			# For rho_delta
			rho_delta_dist = rho_deltaPriorInfo[0];
			rho_delta = rho_delta_dist('rho_delta', *rho_deltaPriorInfo[1:], shape = self._p);
			beta_delta = -4.0 * tt.log(rho_delta);
			# For lambda_eta
			lambda_eta = lambda_etaPriorInfo[0]('lambda_eta', *lambda_etaPriorInfo[1:]);
			# For lambda delta
			lambda_delta = lambda_deltaPriorInfo[0]('lambda_delta', *lambda_deltaPriorInfo[1:]);
			# For lambda_epsi
			lambda_epsi = lambda_epsiPriorInfo[0]('lambda_epsi', *lambda_epsiPriorInfo[1:]);
			# Construct the data
			...
			getCovMat = covFuncMapping[covFuncName];	
	...
	def run(self, pymc3Model, draws, sampler, njobs):
		trace = None;
		if sampler.lower() == 'metropolis':
			with pymc3Model:
				trace = sample(draws, step = Metropolis(), tune = int(draws * 0.25), njobs = njobs);
		elif sampler.lower() == 'nuts':
			with pymc3Model:
				trace = sample(draws, njobs = njobs);
		return trace;
