...
model {
	...
	theta[1] ~ normal(0.4, 0.3); //piched roof insul thickness
	theta[2] ~ normal(0.2, 0.3); //ext win U
	theta[3] ~ normal(0.1, 0.3); //ext win SHGC
	theta[4] ~ normal(0.6, 0.3); //infil rate
	theta[5] ~ normal(0.1, 0.3); //mull surface metal thickness
